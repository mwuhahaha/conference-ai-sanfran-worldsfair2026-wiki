...
- Third-party connections must pass separate identity and event-association gates before credibility/relevance scoring can influence review priority. A shared name, repository name, guessed domain, exact shared profile URL, or popularity signal is never sufficient proof. Publication as labeled comparison/context also does not mean the wiki endorses the source or its claims.
