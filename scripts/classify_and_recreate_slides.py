...
WIKI = ROOT / "wiki"
SLIDE_ASSETS = WIKI / "assets" / "slides"
...
def slide_images(video_id: str, args: argparse.Namespace) -> list[Path]:
    if args.slide:
        ...
        for name in args.slide:
            path = SLIDE_ASSETS / video_id / name
            if path.exists():
                found.append(path)
        return found[: args.limit or None]
    slides = sorted((SLIDE_ASSETS / video_id).glob("*.jpg"))
    return slides[: args.limit or None]


def ocr_path(video_id: str, slide: Path) -> Path:
    return SLIDE_OCR / video_id / f"{slide.stem}.txt"


def classification_path(video_id: str, slide: Path) -> Path:
    return CLASSIFICATION_ROOT / video_id / f"{slide.stem}.json"


def recreation_path(video_id: str, slide: Path) -> Path:
    return RECREATION_ASSETS / video_id / f"{slide.stem}.html"
...
def render_recreation(video_id: str, slide: Path, result: dict) -> str:
...
<head>
  ...
  <title>Slide recreation {html.escape(video_id)} {html.escape(slide.stem)}</title>
  <style>
...
def refresh_standard_page(video_id: str, accepted: list[dict], rejected: list[dict]) -> None:
    page = SLIDE_PAGES / f"youtube-{video_id}-slides.md"
    if not page.exists():
        return
    lines = []
    if not accepted:
        lines.append("No slide-like frames were retained after AI slide classification.")
    for item in accepted:
        ...
        text = item.get("text") or read_text(ocr_path(video_id, slide))
        if text:
            lines.extend(["", "Slide text:", "", "> " + text.strip().replace("\n", "\n> "), ""])
    if rejected:
        lines.extend(["", "## Removed Non-Slide Frames"])
        for item in rejected:
            lines.append(f"- `{Path(item['image']).name}` — `{item['frame_type']}` confidence `{item['confidence']}`; {item.get('reject_reason') or 'not a content slide'}")
...
def classify_video(video_id: str, args: argparse.Namespace) -> dict:
    ...
    for slide in slide_images(video_id, args):
        cached = classification_path(video_id, slide)
        if args.reuse and cached.exists():
            result = json.loads(read_text(cached))
        else:
            result = codex_cli(slide, read_text(ocr_path(video_id, slide)), args.model, args.timeout)
        ...
        if keep:
            recreation = recreation_path(video_id, slide)
            write_text(recreation, render_recreation(video_id, slide, result))
            record["recreation"] = str(recreation)
            accepted.append(record)
        else:
            rejected.append(record)
            if args.remove_rejected:
                for path in [slide, ocr_path(video_id, slide)]:
                    if path.exists():
                        path.unlink()
    if not args.no_refresh_page:
        refresh_standard_page(video_id, accepted, rejected)
    audit = {
        "video_id": video_id,
        "model": args.model,
    ...
    write_text(CLASSIFICATION_ROOT / video_id / "audit.json", json.dumps(audit, indent=2, ensure_ascii=False))
    return audit
...
def main() -> int:
    ...
    parser.add_argument("--remove-rejected", action="store_true", help="Remove rejected images from wiki/assets/slides.")
