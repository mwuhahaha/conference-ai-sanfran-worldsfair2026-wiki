...
function connectedCategoryNodes(name){if(!name)return null;const seeds=graph.filterNodes((node,a)=>a.category===name),visible=new Set(seeds);seeds.forEach(node=>graph.forEachNeighbor(node,neighbor=>visible.add(neighbor)));return visible}
...
function setCategory(name){category.value=name;categoryNodes=connectedCategoryNodes(name);selected='';detail.classList.remove('open');const url=new URL(location.href);name?url.searchParams.set('category',name):url.searchParams.delete('category');history.replaceState({},'',url);renderer.refresh();requestAnimationFrame(fitCategory);update()}
const requested=new URLSearchParams(location.search).get('category');if(requested&&categories.includes(requested)){category.value=requested;categoryNodes=connectedCategoryNodes(requested)}
