---
title: "Zions Bancorporation"
category: "companies"
aliases: ["Zions Bancorporation"]
website: "https://www.zionsbancorporation.com/"
sourceLabels: ["Official speaker roster", "Official conference schedule", "Public company site", "Manual company URL override", "Automated company profile fetch"]
---
# Zions Bancorporation

## Overview
Zions Bancorporation is represented at AI Engineer World's Fair 2026 through the official roster and related scheduled sessions. A known public site was attached for source navigation, but automated metadata extraction was unavailable in this pass (manual_url_unfetched).

## Background
The official speaker roster connects Zions Bancorporation to Tanmay Sah. A manual URL override attaches the public site for source navigation because automated discovery or metadata extraction was inconclusive.

## Conference Relevance
Zions Bancorporation matters to this wiki through 1 official speaker(s) and 2 scheduled session(s), including: 2 hr deep dive on LLM Inference at Scale — Part 1 of 2; 2 hr deep dive on LLM Inference at Scale — Part 2 of 2.

## Conference Sessions
- [[2026-06-29-harshul-jain-2-hr-deep-dive-on-llm-inference-at-scale-part-1-of-2]] - 2 hr deep dive on LLM Inference at Scale — Part 1 of 2 (2026-06-29, 12:10pm-1:10pm)
- [[2026-06-29-harshul-jain-2-hr-deep-dive-on-llm-inference-at-scale-part-2-of-2]] - 2 hr deep dive on LLM Inference at Scale — Part 2 of 2 (2026-06-29, 1:15pm-2:15pm)

## Connections
- [[tanmay-sah]] - Senior Quantitative Modeler

## Sources
- [Zions Bancorporation public site](https://www.zionsbancorporation.com/)

## Evidence Graph
This evidence graph summarizes how this organization appears across the conference source graph: scheduled sessions, linked videos, transcripts, and slide-derived evidence.

### Linked Sessions
- [[2026-06-29-harshul-jain-2-hr-deep-dive-on-llm-inference-at-scale-part-1-of-2|2 hr deep dive on LLM Inference at Scale — Part 1 of 2]]
- [[2026-06-29-harshul-jain-2-hr-deep-dive-on-llm-inference-at-scale-part-2-of-2|2 hr deep dive on LLM Inference at Scale — Part 2 of 2]]

### Media Signals
No linked video, transcript, or slide source has been attached yet.

## Notes
- Automated company profile fetch status: manual_url_unfetched.
- Discovery confidence score: 50.

## Evidence Boundary
Official roster and schedule facts are treated as canonical for conference participation. Public company sites, documentation, and professional profiles are supporting context used to explain what the organization does and why it is relevant.
