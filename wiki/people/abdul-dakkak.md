---
title: Abdul Dakkak
category: people
role: Chief Scientist
company: Modular
sourceLabels:
  - Official speaker roster
  - Official conference schedule
last_auto_summarized: '2026-07-06T08:42:40.867Z'
---
# Abdul Dakkak

## Official Role
Chief Scientist at Modular.

## Official Bio
Abdul Dakkak is Chief Scientist at Modular, where his work sits at the center of AI compute and GenAI performance. In the World's Fair program, he is tied to Modular's hardware and systems story: helping AI teams deal with the expanding mix of accelerators, kernels, framework layers, and serving infrastructure behind modern model deployment. His role connects Mojo, low-level performance work, and Modular's broader platform strategy for making AI workloads run efficiently across changing hardware targets.

## Scheduled Sessions
- [[2026-06-30-abdul-dakkak-modular-taming-the-ai-hardware-cambrian-explosion]] — Modular: Taming the AI Hardware Cambrian Explosion (2026-06-30, 3:45pm-4:05pm)

## Links
- [LinkedIn](https://www.linkedin.com/in/adakkak)
