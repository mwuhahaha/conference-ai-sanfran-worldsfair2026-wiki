---
title: Laurie Voss
category: people
role: Head of Developer Relations
company: Arize AI
linkedin: 'https://www.linkedin.com/in/seldo/'
twitter: 'https://x.com/seldo'
website: 'https://seldo.com'
blog: 'https://seldo.com'
sourceLabels:
  - Official speaker roster
  - Official conference schedule
last_auto_summarized: '2026-07-18T21:12:34.125Z'
sourceAssessment:
  schemaVersion: 1
  claimId: claim:3f3b6bf49f3b5f8df63d43cad7946ed593fafc780968adc17975b961b37c127b
  subjectId: person:laurie-voss
  domain: people page evidence coverage
  intendedUse: attributed_context
  asOf: '2026-07-24T00:00:00.000000Z'
  state: limited
  basis: official_primary_canonical
  message: This page is limited to source-attributed facts; independent support for broader claims may be limited.
  publicSourceIds:
  - source:official-wf26-official-speakers
sourceAssessmentBodySha256: sha256:57f4ef55b237b37fd81fc78e3406d5fbe8c37e5822a6ddab15f994d80ac76e44
---
# Laurie Voss

## Profile
Laurie Voss is Head of Developer Relations at [[arize-ai|Arize AI]], where he focuses on the evaluation and observability practices needed to move agents from convincing demos into dependable production systems. His five confirmed World’s Fair appearances connect that work across two hands-on [[agent-evaluations|agent evaluation]] workshops, the Evals track introduction, a session on how [[coding-agents|coding agents]] reshape code review, and a [[mcp|context-engineering]] talk about how instruction length affects an agent’s ability to retain and apply skills. Together, the sessions position evaluation as part of the full engineering loop: tracing behavior, constructing datasets, judging outputs, reviewing agent-produced changes, and controlling the context available at inference time.

- [LinkedIn](https://www.linkedin.com/in/seldo/)
- [X / Twitter](https://x.com/seldo)
- [Website](https://seldo.com)
- [Blog](https://seldo.com)

## Biography
Voss has worked as a developer for more than 30 years and co-founded npm, Inc., giving him direct experience with infrastructure shifts that change how software is built, distributed, and reviewed. At [[arize-ai|Arize AI]], he brings that perspective to AI observability and [[agent-evaluations|agent evaluation]], including production traces, representative datasets, automated and human judges, output-quality analysis, and the question of whether an agent remains reliable beyond a successful demonstration. The linked “Ship Real Agents” transcript and slide material reinforces this practical emphasis through detailed treatment of evals, tracing, datasets, judges, and Phoenix, while remaining supporting context rather than a verified World’s Fair 2026 recording. His confirmed program extends the same production discipline into [[autoresearch|autoresearch]], agent-driven code-review workflows, and context limits: how much instruction a skill can carry before an agent forgets or fails to apply it. This connects his conference role to the broader concerns of [[coding-agents|coding agents]], [[mcp|Model Context Protocol]], and [[inference-engineering|inference engineering]]—measuring behavior, supplying usable context, and making model capability dependable inside real systems.

## Conference Sessions
- [[2026-06-29-laurie-voss-from-vibes-to-production-evaluating-and-shipping-ai-agents-that-work-101]] — From Vibes to Production: Evaluating and Shipping AI Agents That Work 101 (2026-06-29, 9:00am-11:00am)
- [[2026-06-29-laurie-voss-from-vibes-to-production-evaluating-and-shipping-ai-agents-that-work-201]] — From Vibes to Production: Evaluating and Shipping AI Agents That Work 201 (2026-06-29, 2:20pm-4:20pm)
- [[2026-06-30-laurie-voss-evals-track-intro]] — Evals Track Intro (2026-06-30, 10:25am-10:30am)
- [[2026-06-30-laurie-voss-the-death-of-the-code-review]] — The Death of the Code Review (2026-06-30, 12:05pm-12:25pm)
- [[2026-06-30-laurie-voss-how-long-can-your-skills-be-before-your-agent-forgets-what-you-told-it]] — How long can your skills be before your agent forgets what you told it? (2026-06-30, 1:30pm-1:50pm)

## Evidence Graph
This section summarizes how this person appears across the conference source graph: scheduled sessions, linked videos, transcripts, and slide-derived evidence.

Event participation, role, and affiliation details remain attributed to the official event program; publication here is not an endorsement.

### Linked Sessions
- [[2026-06-29-laurie-voss-from-vibes-to-production-evaluating-and-shipping-ai-agents-that-work-101|From Vibes to Production: Evaluating and Shipping AI Agents That Work 101]]
- [[2026-06-29-laurie-voss-from-vibes-to-production-evaluating-and-shipping-ai-agents-that-work-201|From Vibes to Production: Evaluating and Shipping AI Agents That Work 201]]
- [[2026-06-30-laurie-voss-evals-track-intro|Evals Track Intro]]
- [[2026-06-30-laurie-voss-the-death-of-the-code-review|The Death of the Code Review]]
- [[2026-06-30-laurie-voss-how-long-can-your-skills-be-before-your-agent-forgets-what-you-told-it|How long can your skills be before your agent forgets what you told it?]]

### Media Signals
- `youtube-Xfl50508LZM` — 22,591 transcript words; 10 slide-derived text signals; role: supporting context only.
- Transcript signals for `youtube-Xfl50508LZM`: evals, eval, data, should, judge, output, whether, phoenix.
- Slide-derived themes for `youtube-Xfl50508LZM`: swiss, cheese, talking, setting, tracing, phoenix, paine, theoretical.
- Evidence links for `youtube-Xfl50508LZM` (supporting context only): [[youtube-Xfl50508LZM]], [[youtube-Xfl50508LZM-transcript]], [[youtube-Xfl50508LZM-slides]], [[youtube-Xfl50508LZM-dense-slides]], [[youtube-Xfl50508LZM-reconstructed-slides]]
