---
title: Mike Chambers
category: people
role: Senior Developer Advocate for Generative AI
company: Amazon Web Services (AWS)
blog: 'https://blog.mikegchambers.com'
sourceLabels:
  - Official speaker roster
  - Official conference schedule
last_auto_summarized: '2026-07-18T13:16:40.861Z'
sourceAssessment:
  schemaVersion: 1
  claimId: claim:2b85685b77e3068858ce39c8d0665dd0c342b708d677bdbbfcc4ce0aefdbe367
  subjectId: person:mike-chambers
  domain: people page evidence coverage
  intendedUse: attributed_context
  asOf: '2026-07-20T01:11:38.017560Z'
  state: limited
  basis: official_primary_canonical
  message: This page is limited to source-attributed facts; independent support for broader claims may be limited.
  publicSourceIds:
  - source:official-wf26-official-speakers
sourceAssessmentBodySha256: sha256:a429fa790b34cac225e170a6fb369ae53b206ec551f175e2386dfa5be280d004
---
# Mike Chambers

## Profile
Mike Chambers is a Senior Developer Advocate for Generative AI at [[amazon-web-services-aws|Amazon Web Services (AWS)]]. His work focuses on moving agentic systems from demonstrations into production through practical harnesses, operational safeguards, and repeatable engineering workflows.

- [Blog](https://blog.mikegchambers.com)

## Biography
Chambers creates hands-on educational material about agentic AI and Amazon Bedrock, including serverless agent workflows and Generative AI with Large Language Models. At World’s Fair 2026, he brings that production-oriented perspective to harness engineering: the surrounding “cage” that gives powerful domain agents the context, constraints, evaluation, and operational structure needed for real-world applications. His linked supporting talk, “Ship it! Building Production Ready Agents,” reinforces this emphasis and connects his work to the broader [[software-factories|software-factory]] model of repeatable agent delivery.

## Conference Sessions
- [[2026-07-01-mike-chambers-harness-engineering-building-the-production-cage-for-powerful-domain-agents]] — Harness Engineering: Building the Production Cage for Powerful Domain Agents (2026-07-01, 12:05pm-12:25pm)

## Evidence Graph
This section summarizes how this person appears across the conference source graph: scheduled sessions, linked videos, transcripts, and slide-derived evidence.

Event participation, role, and affiliation details remain attributed to the official event program; publication here is not an endorsement.

### Linked Sessions
- [[2026-07-01-mike-chambers-harness-engineering-building-the-production-cage-for-powerful-domain-agents|'Harness Engineering: Building the Production Cage for Powerful Domain Agents']]

### Media Signals
- `youtube-HT4l0DeP69I` — 10 slide-derived text signals; role: supporting context only.
- Slide-derived themes for `youtube-HT4l0DeP69I`: mike, chambers, advocate, engineering, generative, real, world, applications.
- Evidence links for `youtube-HT4l0DeP69I` (supporting context only): [[youtube-HT4l0DeP69I]], [[youtube-HT4l0DeP69I-slides]], [[youtube-HT4l0DeP69I-dense-slides]], [[youtube-HT4l0DeP69I-reconstructed-slides]]
## Livestream Appearances
- [[2026-07-01-mike-chambers-harness-engineering-building-the-production-cage-for-powerful-domain-agents|Harness Engineering: Building the Production Cage for Powerful Domain Agents]] — [watch at 03:14:28](https://www.youtube.com/watch?v=I2cbIws9j10&t=11668s) in WF26: Harness Engineering & Startup Battlefield.
