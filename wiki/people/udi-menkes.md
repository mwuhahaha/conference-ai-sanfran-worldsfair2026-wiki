---
title: "Udi Menkes"
category: "people"
role: "Principal Product Manager"
company: "Intuit"
linkedin: "https://www.linkedin.com/in/udimenkes/"
twitter: "https://x.com/menkesu"
website: "https://genaipm.com/about"
blog: "https://genaipm.com/about"
sourceLabels: ["Official speaker roster", "Official conference schedule"]
sourceAssessment:
  schemaVersion: 1
  claimId: claim:329e598bad3782877ba25cf417b58c20e598bca56ad922fc4784b6e08dc13ea0
  subjectId: person:udi-menkes
  domain: people page evidence coverage
  intendedUse: attributed_context
  asOf: '2026-07-24T00:00:00.000000Z'
  state: limited
  basis: official_primary_canonical
  message: This page is limited to source-attributed facts; independent support for broader claims may be limited.
  publicSourceIds:
  - source:official-wf26-official-speakers
sourceAssessmentBodySha256: sha256:ec86ac083b934e45b09ce65668197cc8aa1a51f4f18127e8a48b3e6f57e823bf
---
# Udi Menkes

## Profile
Principal Product Manager at [[intuit|Intuit]].

- [LinkedIn](https://www.linkedin.com/in/udimenkes/)
- [X / Twitter](https://x.com/menkesu)
- [Website](https://genaipm.com/about)
- [Blog](https://genaipm.com/about)

## Biography
Udi Menkes is a Principal AI Product Manager at [[intuit|Intuit]], where he leads the development of custom financial LLMs and agentic AI systems that turn [[intuit|Intuit]]'s proprietary data and financial domain expertise into experiences delivering real value to 100M+ customers across QuickBooks, TurboTax. Credit Karma, and Mailchimp. He has spent 15+ years building AI that touches money — patented financial crime AI at ThetaRay for banks including Citi, Santander, and OCBC, and conversational and revenue-optimization AI at OpenWeb for 120M+ monthly users. Udi runs GenAI PM (genaipm.com), an AI agent that scans X, LinkedIn, YouTube, GitHub, and 200+ AI and PM blogs daily and delivers a 7-minute, high-signal brief to 5,500+ AI Product teams and builders at Google, OpenAI, NVIDIA, Meta, Amazon, Microsoft, and more.

## Conference Sessions
- [[2026-07-01-udi-menkes-why-off-the-shelf-ai-doesn-t-understand-money]] — Why Off-the-Shelf AI Doesn't Understand Money (2026-07-01, 11:10am-11:30am)

## Evidence Graph
This section summarizes how this person appears across the conference source graph: scheduled sessions, linked videos, transcripts, and slide-derived evidence.

Event participation, role, and affiliation details remain attributed to the official event program; publication here is not an endorsement.

### Linked Sessions
- [[2026-07-01-udi-menkes-why-off-the-shelf-ai-doesn-t-understand-money|Why Off-the-Shelf AI Doesn't Understand Money]]

### Media Signals
No linked video, transcript, or slide source has been attached yet.
