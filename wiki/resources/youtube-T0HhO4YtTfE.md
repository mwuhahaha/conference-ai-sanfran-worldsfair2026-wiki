---
title: "AI System Design: From Idea to Production - Apoorva Joshi, MongoDB"
category: "resources"
sourceLabels:
  - "Public YouTube metadata"
  - "YouTube transcript"
videoId: "T0HhO4YtTfE"
last_enriched: "2026-07-09T01:18:24.863412+00:00"
---
# AI System Design: From Idea to Production - Apoorva Joshi, MongoDB

## What It Is
A public AI Engineer YouTube AI Engineer cut video used as supporting material for the AI Engineer World's Fair 2026 wiki.

## Transcript Status
Cached transcript text is available at `raw/sources/youtube-transcripts/T0HhO4YtTfE.txt` (4,200 words).

## Topic Signals
- [[coding-agents|Coding Agents]], [[agentic-search|Agentic Search]], [[agent-evaluations|Agent Evaluations]], [[agent-memory|Agent Memory]]
- Transcript keywords: `deny`, `reside`, `exact`, `leave`, `faithfulness`, `resource`, `manual`, `proactively`

## Link
[YouTube](https://www.youtube.com/watch?v=T0HhO4YtTfE)

## Extracted Slides
- [[youtube-T0HhO4YtTfE-slides]]

## Transcript Markdown
- [[youtube-T0HhO4YtTfE-transcript]] — full cached transcript markdown.
