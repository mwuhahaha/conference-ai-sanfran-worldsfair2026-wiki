---
title: "We Cut 94% of AI Coding Tokens With a Local Code Index - Rajkumar Sakthivel, Tesco"
category: "resources"
sourceLabels:
  - "Public YouTube metadata"
  - "YouTube transcript"
videoId: "dRmWYHuIJxM"
last_enriched: "2026-07-09T01:18:28.659846+00:00"
---
# We Cut 94% of AI Coding Tokens With a Local Code Index - Rajkumar Sakthivel, Tesco

## What It Is
A public AI Engineer YouTube AI Engineer cut video used as supporting material for the AI Engineer World's Fair 2026 wiki.

## Transcript Status
Cached transcript text is available at `raw/sources/youtube-transcripts/dRmWYHuIJxM.txt` (1,320 words).

## Topic Signals
- [[coding-agents|Coding Agents]], [[mcp|MCP]], [[inference-engineering|Inference Engineering]], [[agentic-search|Agentic Search]]
- Transcript keywords: `exact`, `tested`, `takes`, `paying`, `estimate`, `screen`, `every`, `confident`

## Link
[YouTube](https://www.youtube.com/watch?v=dRmWYHuIJxM)

## Extracted Slides
- [[youtube-dRmWYHuIJxM-slides]]

## Transcript Markdown
- [[youtube-dRmWYHuIJxM-transcript]] — full cached transcript markdown.
