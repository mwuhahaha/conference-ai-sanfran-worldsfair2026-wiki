...
# Spin at the Gate Until Green: The Engineering Primitives Behind Self-Driving Codebases ## Conference Context - Date/time: 2026-06-29 · 1:30pm-1:50pm - Track/room: Software Factories · Leadership 1 - Speaker(s): Andrew Orobator - Session type/status: session · confirmed - Track: Software Factories - Room: Leadership 1 - Session type: session - Status: confirmed ## Session Description Most AI-assisted development fails the same way: the AI produces plausible output, the human can't tell if it's right, so they check manually, find the problem, re-prompt, and repeat. This loop doesn't scale. There's a different approach. If you can express correctness as a binary — does it compile, do the tests pass, does the lint check clear — you can remove the human from that loop entirely.
