---
title: "Dagger"
category: "tools"
aliases: ["Dagger"]
sourceLabels: ["Public YouTube metadata", "Local slide OCR"]
scheduleTracks: []
scheduleRooms: []
sourceAssessment:
  schemaVersion: 1
  claimId: claim:8b02220aff496755e4a09c96f9dd5741e9c3b711bba707e65f5798ae1968e5ff
  subjectId: tool:dagger
  domain: tools page evidence coverage
  intendedUse: attributed_context
  asOf: '2026-07-24T00:00:00.000000Z'
  state: pending
  basis: no_admitted_evidence
  message: This page has not yet received enough admitted evidence for a source assessment.
  publicSourceIds: []
sourceAssessmentBodySha256: sha256:f2ba5fc58099a9ea059ccfbbf790814a229a98ee4c18e98059f1acc840b86210
---

# Dagger

## Conference Context
Dagger appears as a high-confidence tool, platform, model, protocol, product, or service in the AI Engineer World's Fair 2026 corpus.

This page records confirmed mentions and keeps them separate from broader inferred relevance.

## Confirmed Evidence

### Public YouTube Metadata
- [YouTube Fzb1a24hF-o](https://www.youtube.com/watch?v=Fzb1a24hF-o) — Ship Agents that Ship: A Hands-On Workshop - Kyle Penfound, Jeremy Adams, Dagger
- [YouTube bUBF5V6oDKw](https://www.youtube.com/watch?v=bUBF5V6oDKw) — Containing Agent Chaos — Solomon Hykes, Dagger

### Resource Pages
- [[advanced-slide-logic-page-audit]]
- [[talk-video-transcript-map]]
- [[youtube-Fzb1a24hF-o]]

### Slide/OCR Pages
- [[youtube-Fzb1a24hF-o-dense-slides]]
- [[youtube-Fzb1a24hF-o-reconstructed-slides]]
- [[youtube-Fzb1a24hF-o-slides]]

## Confidence
High confidence for presence in the corpus. Interpret broader importance through the linked schedule, transcript, slide, and topic evidence.
