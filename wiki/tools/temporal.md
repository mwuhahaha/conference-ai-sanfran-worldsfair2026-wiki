---
title: "Temporal"
category: "tools"
aliases: ["Temporal"]
sourceLabels: ["Official schedule", "Public YouTube metadata", "Transcript evidence", "Local slide OCR"]
scheduleTracks: ["Evals", "Graphs"]
scheduleRooms: ["Track 5", "Expo Stage 3 SW"]
sourceAssessment:
  schemaVersion: 1
  claimId: claim:423bcd9a4cfaa5fb61502580ba34e7324aaf82dcf583ac35a2341423a0ceb526
  subjectId: tool:temporal
  domain: tools page evidence coverage
  intendedUse: attributed_context
  asOf: '2026-07-24T00:00:00.000000Z'
  state: limited
  basis: official_primary_canonical
  message: This page is limited to source-attributed facts; independent support for broader claims may be limited.
  publicSourceIds:
  - source:official-wf26-official-sessions
sourceAssessmentBodySha256: sha256:04f21a765842beea618f4219be9ed897629b0e7cecac27656b0a5bd68db604b6
---

# Temporal

## Conference Context
Temporal appears as a high-confidence tool, platform, model, protocol, product, or service in the AI Engineer World's Fair 2026 corpus.

This page records confirmed mentions and keeps them separate from broader inferred relevance.

## Confirmed Evidence

### Official Schedule
- [[2026-06-30-maor-bril-evaling-video-slop]] — Evaling Video Slop (Evals / Track 5)
- [[2026-07-01-james-le-video-has-no-memory-here-s-how-we-built-one]] — Video Has No Memory. Here's How We Built One. (Graphs / Track 5)
- [[2026-07-01-melanie-warrick-the-human-is-an-async-api]] — The Human Is an Async API (track TBD / Expo Stage 3 SW)

### Public YouTube Metadata
- [YouTube k8cnVCMYmNc](https://www.youtube.com/watch?v=k8cnVCMYmNc) — OpenAI + @Temporalio : Building Durable, Production Ready Agents - Cornelia Davis, Temporal
- [YouTube qLqttdO33UM](https://www.youtube.com/watch?v=qLqttdO33UM) — Vision: Zero Bugs — Johann Schleier-Smith, Temporal
- [YouTube 1izYWsokr9s](https://www.youtube.com/watch?v=1izYWsokr9s) — Scaling AI Agents Without Breaking Reliability — Preeti Somal, Temporal
- [YouTube KJ9eZYTWS1Y](https://www.youtube.com/watch?v=KJ9eZYTWS1Y) — Events are the Wrong Abstraction for Your AI Agents - Mason Egger, Temporal.io

### Resource Pages
- [[advanced-slide-logic-page-audit]]
- [[aie-wiki-generation-delta]]
- [[talk-video-transcript-map]]
- [[youtube-k8cnVCMYmNc]]

### Slide/OCR Pages
- [[youtube-5_QWh4LGoxg-slides]]
- [[youtube-T5IMo5ntyhA-slides]]
- [[youtube-k8cnVCMYmNc-dense-slides]]
- [[youtube-k8cnVCMYmNc-reconstructed-slides]]
- [[youtube-k8cnVCMYmNc-slides]]
- [[youtube-qdZzND79mcg-slides]]

### Transcript Files
- `raw/sources/youtube-transcripts/IJXjTLPzvAU.txt`
- `raw/sources/youtube-transcripts/TUnPNY4E2fw.txt`

## Confidence
High confidence for presence in the corpus. Interpret broader importance through the linked schedule, transcript, slide, and topic evidence.
